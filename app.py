...
# -----------------------------
# Load spaCy model safely (cached)
# -----------------------------
@st.cache_resource(show_spinner="Loading language model...")
def load_spacy_model():
    """
    Loads the English spaCy model safely for Streamlit deployment.
    Ensures caching to avoid reloads on every rerun.
    """
    try:
        return spacy.load("en_core_web_sm")
    except OSError:
        st.error(
            "spaCy language model 'en_core_web_sm' not found. "
            "Please ensure it is installed via requirements.txt"
        )
        st.stop()
